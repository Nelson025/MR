...
setwd("F:\\糖尿病_先兆子痫\\R10\\T2D\\01.exposure")      #设置工作目录

#############################################04.exposure----
#读取输入文件, 并对输入文件进行格式转换
#### 1.加载结局数据 ----
load('finndataR10_T2D_exposure.rdata')
data=finndata

#根据pvalue<5e-08对结果进行过滤
outTab<-subset(data, pval.exposure<5e-08)
#outTab<-subset(data, data$pval.exposure<5e-08)
...
out <- ieugwasr::ld_clump(dplyr::tibble(rsid=exposure_dat$SNP, pval=exposure_dat$pval.exposure,id = exposure_dat$id.exposure),
                          clump_kb = 10000,
                          clump_r2 = 0.001,
                          clump_p = 1,
                          plink_bin = plinkbinr::get_plink_exe(),
                          bfile = "E:/S_生信/生信_孟德尔/ld_clump/1kg.v3/EUR",
                          pop = "EUR")
keep <- paste(exposure_dat$SNP,exposure_dat$id.exposure) %in% paste(out$rsid,out$id)
exposure_dat_clumped <- exposure_dat[keep,]
