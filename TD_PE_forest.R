...
selectMethod=c("MR Egger","Weighted median","Inverse variance weighted", "Simple mode","Weighted mode")     #选择展示的方法
setwd("F:\\糖尿病_先兆子痫\\R10\\T1_T2_forest")      #设置工作目录
files=dir()                           #获取目录下所有文件
...
#准备图形参数
tm <- forest_theme(base_size = 20,   #图形的大小
...
plot <- forestploter::forest(data[, c("exposure","nsnp","method","pval"," ","OR(95% CI)")],
                             ...
                             ref_line = 1,      #参考线条的位置
                             xlim = c(0.75, 1.35),    #X轴的范围
                             ticks_at = c(0.8,0.9,1,1.1,1.2),  #ticks_at参数设置坐标轴的刻度位置,大小对应xaxis_cex
                             theme = tm,        #图形的参数
)
...
plot <- add_border(plot, part = "header", row = c(1,6,11), gp = gpar(lwd =1)) #上下边框线
#设置字体大小, 并且让文字居中
...
#输出图形
pdf("forest2.pdf", width=15, heigh=11)
print(plot)
dev.off()
